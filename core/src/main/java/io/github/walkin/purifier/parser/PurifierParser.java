package io.github.walkin.purifier.parser;
...
import io.github.walkin.purifier.name.*;
import io.github.walkin.purifier.util.antlr4.ThrowingErrorListener;
 ...
 */
@ThreadSafe
public class PurifierParser {

    // Caches parsed filter expressions
    private static final Cache<String, List<PurifierNode>> CACHE;

    private static final PurifierMetricsSource METRICS_SOURCE;

    static {
        CACHE = CacheBuilder.from(PurifierConfig.getParserNodeCacheSpec()).build();
        METRICS_SOURCE = new GuavaCachePurifierMetricsSource("squiggly.parser.nodeCache.", CACHE);
    }
     ...
     */
    public List<PurifierNode> parse(String filter) {
        ...
        List<PurifierNode> cachedNodes = CACHE.getIfPresent(filter);
        ...
        List<PurifierNode> nodes = Collections.unmodifiableList(visitor.visit(parser.parse()));
    ...
    private class Visitor extends SquigglyExpressionBaseVisitor<List<PurifierNode>> {
        @Override
        public List<PurifierNode> visitParse(SquigglyExpressionParser.ParseContext ctx) {
    ...
    private class MutableNode {
        ...
        PurifierNode toSquigglyNode() {
            if (name == null) {
                throw new IllegalArgumentException("No Names specified");
            }

            List<PurifierNode> childNodes;
        ...
        private PurifierNode newSquigglyNode(SquigglyName name, List<PurifierNode> childNodes) {
            return new PurifierNode(name, childNodes, negated, squiggly, emptyNested);
        }
